...
try:
    ...
    st.error("❌ Không tìm thấy file '.pkl'. Vui lòng để chung thư mục với file code này.")
    st.stop()
...
if selected_game != "":
    ...
    # Hiển thị thông tin tổng quan của game được chọn bằng các Widget trực quan
    st.write("---")
    st.subheader(f"🎮 Bạn đã chọn: {selected_game}")
    
    col1, col2, col3 = st.columns(3)
    with col1:
        st.info(f"**Thuộc Bang Phái Số:** Nhóm {game_cluster}")
    with col2:
        st.warning(f"**Độ Khó Trò Chơi gốc:** {round(target_complexity, 2)} / 5")
    with col3:
        st.success(f"**Tổng số ứng viên trong bang:** {len(cluster_indices)} games")
